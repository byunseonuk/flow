...
exports.customer_info = async function(mcode){
    ...
    let mtxt = '';
    console.log("entered in customer.js");
    switch(mcode.substring(1,2)){
        case('0'):          // quickReplies
        // await this.customer_info0(mcode);
        return this.customer_info0(mcode);
            break;
        case('1'):          // 거래처 상세정보
        mtxt = mcode.split("/")
    ...
    console.dir(json);
    console.log(json.quickReplies);
    console.log(json.lines);
    console.log(json.lines.cards);
...
exports.customer_info0 = async function(mcode){
    qrp = [], json = [], card = [];
    console.log("enter in customer_info0");
    if(mcode.length<=2){            // 퀵리플라이 보내기
        console.log("enter in mcode.length<=2");
        // try {
        ...
        // let query = await srv.run(           // 던지는 쿼리
        //     SELECT.from(entity)
        //     .where({pernr:'00000001'})
        //     .limit(3)
        //     )
        return await srv.run(           // 던지는 쿼리
        SELECT.from(entity)
        .where({pernr:'00000001'})
        .limit(3)
        )
        ...
        console.log("not enter in mcode.length<=2");
        card = [
            {
                "contents": [
                    {
...
exports.customer_info2 = async function(p_kunnr){
    ...
    console.log("entered in customer_info2");
